...
const TyperWriter: React.FC<Typewriterprops> = ({ custommode, float, custominput, countbytime, repetation, customStyle, setdata, auto }) => {
    ...
    const allchars = useRef({ total: 0, wrong: 0 });
    ...
    useEffect(() => {
        try {
            ...
            const { store, wrongwords, total, correct } = matchText(inputText, originalText, [...textmap]);
    ...
    useEffect(() => {
        ...
        if (!start) return;
        if (initialtime >= reps) {
            let data = wpm(initialtime, correctchars.current, totalchars.current, allchars.current.total, allchars.current.wrong);
